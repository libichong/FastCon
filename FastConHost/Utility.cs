...
namespace FastConHost
{
    ...
    {
        public const string FormatStringTB = "{0:N} TB";
        public const string FormatStringGB = "{0:N} GB";
        public const string FormatStringMB = "{0:N} MB";
        public const string FormatStringKB = "{0:N} KB";
        public const string FormatStringB = "{0:N} B";
        /// <summary>
        ...
        public static string FormatSizeString(long size)
        {
            if (size >= 1024 * 1024 * 1024)
                return string.Format(Utility.FormatStringGB, (double)size / (1024 * 1024 * 1024));
            else if (size >= 1024 * 1024)
                return string.Format(Utility.FormatStringMB, (double)size / (1024 * 1024));
            else if (size >= 1024)
                return string.Format(Utility.FormatStringKB, (double)size / 1024);
            else
                return string.Format(Utility.FormatStringB, size);
        }
        ...
        public static byte[] HexStringAsByteArray(string hex)
        {
            ...
            if (hex.Length % 2 != 0)
                throw new Exception("InvalidHexStringLengthError");
            ...
            for (int i = 0; i < hex.Length; i += 2)
            {
                ...
                if (upper < 0)
                    throw new Exception(string.Format("InvalidHexDigitError", hex[i]));
                if (lower < 0)
                    throw new Exception(string.Format("InvalidHexDigitError", hex[i + 1]));
