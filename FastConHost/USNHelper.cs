    ...
    public class USNHelper : IDisposable
    {
        ...
        internal USNHelper(string path, string volumeRoot)
        {
            if (Utility.IsClientLinux)
                throw new Exception("LinuxNotSupportedError");

            if (!System.IO.Path.IsPathRooted(path))
                throw new Exception(string.Format("Path {0} is not rooted", path));

            m_path = Utility.AppendDirSeparator(path);
            ...
            if (this.FileSystemEntries.Count == 0)
                throw new Exception("SafeGuardError");
        }
        ...
        public void EnumerateFilesAndFolders(string rootpath, Utility.EnumerationCallbackDelegate callback)
        {
            ...
            foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                if (r.Key.StartsWith(rootpath, Utility.ClientFilenameStringComparision))
                {
        ...
        public List<string> GetChangedFileSystemEntries(string sourceFolder, long lastUsn)
        {
            ...
            foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                if (r.Value.Usn >= lastUsn && r.Key.StartsWith(sourceFolder, Utility.ClientFilenameStringComparision))
                    result.Add(r.Key);
        ...
        {
            foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                if (r.Key.Equals(entry, Utility.ClientFilenameStringComparision))
        ...
        public List<string> GetRenamedFileSystemEntries(string sourceFolder, long lastUsn)
        {
            List<string> result = new List<string>();
            foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                if (r.Value.Usn >= lastUsn && ((r.Value.Reason & Win32USN.USNReason.USN_REASON_RENAME_NEW_NAME) != 0) && r.Key.StartsWith(sourceFolder, Utility.ClientFilenameStringComparision))
                    result.Add(r.Key);
        ...
        public List<string> GetFileSystemEntries(string sourceFolder)
        {
            List<string> result = new List<string>();
            foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                if (r.Key.StartsWith(sourceFolder, Utility.ClientFilenameStringComparision))
                    result.Add(r.Key);
        ...
        public List<string> FileSystemEntries
        {
            get
            {
                List<string> result = new List<string>();
                foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                    if (r.Key.StartsWith(m_path, Utility.ClientFilenameStringComparision))
                        result.Add(r.Key);
        ...
        public List<string> Files
        {
            get
            {
                List<string> result = new List<string>();
                foreach (KeyValuePair<string, Win32USN.USN_RECORD> r in this.Records)
                    if (r.Key.StartsWith(m_path, Utility.ClientFilenameStringComparision) && (r.Value.FileAttributes & Win32USN.EFileAttributes.Directory) == 0)
                        result.Add(r.Key);
        ...
        private long ExtractUsnEntries(uint bytesRead, IntPtr startOffset, List<KeyValuePair<string, Win32USN.USN_RECORD>> records, out bool moreEntries)
        {
            ...
            if (bytesLeft < 8)
                throw new Exception("EmptyResponseError");
